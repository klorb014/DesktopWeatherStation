...
//#define REFRESH_TIMER 30 //5 minutes in seconds
...
void setup()
{
  ...
  // controller->stateChange(Mode::ACCESS_POINT);
...
void loop()
{
  ...
  // {
    ...
    if (Configuration::changeDetected())
    {
      LOG_ERROR("true");
    }

    Mode newState;
    // Poll for config changes. TODO: find a better way to do this
    if (Configuration::changeDetected() && Configuration::available(&newState) && Configuration::load(&newState, config))
    {
      if (!config->connect())
      {
        controller->stateChange(Mode::ACCESS_POINT);
      ...
      controller->stateChange(Mode::HTTP_SERVER);
      controller->refreshDisplay();
    }
